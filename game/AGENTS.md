...
- Game-board placement pads remain visibly outlined after a tray is placed and stay larger than the tray itself. Tray cards must visibly accommodate all six drink positions in a 3-by-2 layout, and successful drops use a short settle animation before play continues.
- During direct placement, show a board-scale drag preview that stays attached at the player's original grab point and highlight exactly one valid destination pad. A pointer press does not select a tray: movement starts dragging, while a true tap toggles the tray's click-to-place selection state.
- Trays already placed on the board are fixed and cannot be dragged again. Every tray still in the staging area can be dragged to any empty board pad.
- Preserve the exact grab point while dragging a staging tray. Determine the destination from the visible preview's center so the highlighted pad and final placement agree with what the player actually dragged.
- Every board pad must declare its own grid row and column. Placed trays layer over pads without participating in auto-placement, so adding a tray never reflows or renumbers the remaining drop targets.
- Same-color merges animate as a staggered cup transfer into the newly placed destination tray. The source tray remains visible as its cups leave, then plays a distinct empty-tray exit animation after the final cup arrives.
- The merge destination is the connected same-color tray with the highest current cup count. Only ties favor the newly placed tray; animation and reducer logic must use the same destination-selection helper.
- Trays are modeled as six independent drink slots and may contain multiple colors in later levels. On placement, every color shared by orthogonally connected trays resolves independently toward the tray holding the most cups of that color; unmatched cups and cups blocked by destination capacity remain in their original tray.
- The current demo exposes two explicit resettable level modes above the board: a pure-color level for basic merging and a mixed-color level for per-color migration. Refresh restarts the currently selected level; switching modes clears the current run and loads that mode's source queue.
- A level cannot remain interactive with no remaining source or reserve trays. Resolve it as won only after all six required sales; otherwise show an explicit failed-level result with progress and a restart action.
